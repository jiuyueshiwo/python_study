...
class JD(object):
    def __init__(self):
        ...
        #self.cursor.execute("""create view v_goods_info as select goods.*, cate.name as cate_name, brand.name as brand_name from goods left join goods_cates as cate on goods.cate_id=cate.id left join goods_brands as brand on goods.brand_id=brand.id""")
    ...
    def show_goods(self):
        num = self.cursor.execute("""select id, name, cate_name, brand_name, price from v_goods_info"""
            )
        self.show_searched_result(num)

    def search_good_by_name(self):
        good_name = "%" + input("请输入要搜索的商品名称:") + "%"
        num = self.cursor.execute("""select id, name, cate_name, brand_name, price from v_goods_info where name like %s""", [good_name])
        self.show_searched_result(num)
    
    def search_good_by_cates(self):
        cate_name = "%" + input("请输入要搜索的商品种类:") + "%"
        num = self.cursor.execute("""select id, name, cate_name, brand_name, price from v_goods_info where cate_name like %s""", [cate_name])
        self.show_searched_result(num)

    def search_good_by_brands(self):
        brand_name = "%" + input("请输入要搜索的商品品牌:") + "%"
        num = self.cursor.execute("""select id, name, cate_name, brand_name, price from v_goods_info where brand_name like %s""", [brand_name])
        self.show_searched_result(num)
